...
__global__ void spmv_csr_gpu_kernel_texture(CSR csr, unsigned n, cudaTextureObject_t input_vec,
                                    float *output_vec) {
  ...
  for (unsigned i = starting_row; i<end_row; i++) {
    ...
    while (val < val_end) {
      out += *val * tex1Dfetch<float>(input_vec, col_val);//__ldg(&input_vec[col_val]);
