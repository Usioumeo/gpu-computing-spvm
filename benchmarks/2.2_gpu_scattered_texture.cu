...
// size of data_block, so how many consegutive elements to process in a single
// block
#define DATA_BLOCK (384)

__device__ inline unsigned upper_bound(cudaTextureObject_t arr, int size,
                                       unsigned key) {
  int left = 0;
  int right = size;
#pragma unroll 4
...
__device__ inline unsigned upper_bound_next(cudaTextureObject_t arr, int size,
...
__global__ void spmv_csr_gpu_kernel_nnz(const float *__restrict__ val,
  ...
  unsigned assigned_end =
      min(block_start + (DATA_BLOCK * (threadIdx.x + 1)) / BLOCK_THREADS,
          block_end);
  // get rows
  unsigned row = 0;
  for (unsigned i = assigned_start; i < assigned_end;) {
    ...
    unsigned row_end =
        min(tex1Dfetch<unsigned>(row_idx, row + 1), assigned_end);
...
#pragma unroll 10
    for (unsigned i = start; i < block_end; i += BLOCK_THREADS) {
      contributions[i - block_start] =
          val[i] * tex1Dfetch<float>(input_tex, col_idx[i]);
    }
  }

  __syncthreads();

  // accumulate all contributions and write them in the least amount of atomic
  // operations
