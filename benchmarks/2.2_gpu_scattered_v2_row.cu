...
int spmv_csr_gpu_nnz(CSR *csr, unsigned n, float *input_vec,
  ...
  CHECK_CUDA(cudaMemcpy(cuda_extended_row, extended_row,
                        sizeof(float) * csr->nnz, cudaMemcpyHostToDevice));

  TEST_FUNCTION(
      dummy_launcher(gpu_csr, input_vec_gpu, output_gpu, cuda_extended_row));
